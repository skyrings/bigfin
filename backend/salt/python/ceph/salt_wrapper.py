...
def StartMon(monitors):
    return run_state(local, monitors, 'start_ceph_mon', expr_form='list')
...
def AddOSD(cluster_name, minions):
    ...
    for minion, osds in osd_map.iteritems():
        name = minions[minion].get('hostname', _get_short_hostname(minion))
        for osd in osds:
...
def CreatePool(monitor, cluster_name, pool_name, pg_num=0):
