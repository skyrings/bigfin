...
func (c CephApi) GetPGErrorCount(mon string, clusterId uuid.UUID, ctxt string) (int, error) {
	...
	pgMap, pgMapOk := pgSummary["pg"].(map[string]map[string]interface{})
	if !pgMapOk {
		return 0, fmt.Errorf("%s - Failed to fetch number of pgs in error for the cluster %v", ctxt, clusterId)
	}
	errPgCount, errPgCountOk := pgMap["critical"]["count"]
	if !errPgCountOk {
		return 0, fmt.Errorf("%s - Failed to fetch number of pgs in error for the cluster %v", ctxt, clusterId)
	}
	return errPgCount, nil
}
