	...
	if storage_id_str == "" {
		...
		asyncTask := func(t *task.Task) {
			for {
				...
				default:
					...
					t.UpdateStatus("Creating the block storage")
		...
		asyncTask := func(t *task.Task) {
			for {
				...
				default:
					t.UpdateStatus("Started ceph provider block device creation: %v", t.ID)
					t.UpdateStatus("Getting details of storage entoty")
					if err := coll.Find(bson.M{"storageid": *storage_id}).One(&storage); err != nil {
						utils.FailTask("Error getting details of storage entoty", fmt.Errorf("%s - %v", ctxt, err), t)
						return
					}
					blockDevice := models.BlockDevice {
						Name: request.Name,
...
func createBlockStorage(ctxt string, clusterId uuid.UUID, backingStorage string, blockDevice models.BlockDevice, t *task.Task) bool {
	...
	if err != nil || !ok {
		utils.FailTask(fmt.Sprintf("Creation of block storage failed on cluster: %s", clusterId), fmt.Errorf("%s - %v", ctxt, err), t)
		return false
	} else {
