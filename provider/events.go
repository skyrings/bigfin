	...
	if strings.HasSuffix(event.Message, "added to the cluster map") {
		...
		for count := 0; count < 12; count++ {
			...
			if err != nil {
				...
				retrivedMonSuccessfully = true
				break
			}
		}
		if !retrivedMonSuccessfully {
		...
		osds, err := cephapi_backend.GetOSDs(monHostname, event.ClusterId, ctxt)
		if err != nil {
			logger.Get().Error("%s-Error getting OSDs list for cluster: %v. error: %v", ctxt, event.ClusterId, err)
			return event, err
		}

		var osd backend.CephOSD
		for _, o := range osds {
			if o.Uuid.String() == event.EntityId.String() {
				osd = o
				break
			}
		}
...
func osd_state_change_handler(event models.AppEvent, osdname string, ctxt string) (models.AppEvent, error) {
	...
	osds, err := cephapi_backend.GetOSDs(monnode.Hostname, event.ClusterId, ctxt)
	if err != nil {
		logger.Get().Error("%s-Error getting OSDs list for cluster: %v. error: %v", ctxt, event.ClusterId, err)
		return event, err
	}
	...
	if strings.Contains(event.Message, "went down") {
...
func ceph_osd_property_changed_handler(event models.AppEvent, ctxt string) (models.AppEvent, error) {
	...
	if strings.HasSuffix(event.Message, "added to the cluster map") || strings.HasSuffix(event.Message, "removed from the cluster map") {
		event, err = osd_add_or_delete_handler(event, osdname, ctxt)
		if err != nil {
			return event, err
		}
	} else if strings.Contains(event.Message, "went down") || strings.Contains(event.Message, "came up") {

		logger.Get().Debug("Darshan inside osd state change handler")
