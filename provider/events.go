...
func node_down_handler(event models.AppEvent, ctxt string) (models.AppEvent, error) {
	...
	if err == nil {
		if calamariMonNode.NodeId != event.NodeId {
			return event, fmt.Errorf("This node does not have calamari server running. Hence node down is being ignored")
		}
	...
	for _, monNode := range monNodes {
		// avoid starting calamari in the same mon node again.
		if monNode.NodeId == event.NodeId {
			continue
		}
