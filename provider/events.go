...
func ceph_pool_add_handler(event models.AppEvent, ctxt string) (models.AppEvent, error) {
	...
	pools, err := cephapi_backend.GetPools(monnode.Hostname, event.ClusterId, ctxt)
	if err != nil {
		return event, err
	}
	...
	for _, pool := range pools {
		if strconv.Itoa(pool.Id) != event.Tags["service_id"] {
			continue
		}
		...
		event.Message = fmt.Sprintf("Detected addition of new pool %s in cluster %s", pool.Name,
			cluster.Name)
		event.Description = fmt.Sprintf("%s. The new pool will added to skyring.", event.Message)
