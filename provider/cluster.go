...
func (s *CephProvider) ExpandCluster(req models.RpcRequest, resp *models.RpcResponse) error {
	...
	asyncTask := func(t *task.Task) {
		...
		for _, node := range nodes {
			if err := coll.Update(
				...
				bson.M{"$set": bson.M{
					...
					"clusterip4": node_ips[node.NodeId]["cluster"],
					"publicip4":  node_ips[node.NodeId]["public"]}}); err != nil {
				t.UpdateStatus("Error updating node details for :%s", node.NodeId)
				return
			}
		}
		...
		if len(mons) > 0 {
			...
			if err != nil {
				t.UpdateStatus("Failed. Error adding mons")
				return
			}
			if ret_val {
				...
				if err != nil || !ret_val {
					t.UpdateStatus("Failed. Error starting/persisiting mons")
					return
				}
			}
		}
		...
		if err != nil {
			t.UpdateStatus("Failed. Error getting updated nodes for OSD creation")
			return
		}
		t.UpdateStatus("Adding OSDs")
		ret_val, err := addOSDs(*cluster_id, cluster.Name, updated_nodes, new_nodes)
		if err != nil || !ret_val {
			t.UpdateStatus("Failed. Error adding OSDs")
			return
		}
		t.UpdateStatus("Success")
		t.Done()
	...
	if taskId, err := s.GetTaskManager().Run("CEPH-ExpandCluster", asyncTask); err != nil {
