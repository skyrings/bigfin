...
func (s *CephProvider) CreateCluster(req models.RpcRequest, resp *models.RpcResponse) error {
	...
	for _, req_node := range request.Nodes {
		if req_node.Options["mon"] == "Y" || req_node.Options["mon"] == "y" {
			var mon backend.Mon
	...
	ret_val, err := salt_backend.CreateCluster(request.ClusterName, *cluster_uuid, mons)
	if err != nil {
	...
	if ret_val {
		...
		ret_val, _ = addOSDs(*cluster_uuid, request.ClusterName, updated_nodes, request.Nodes)
		if !ret_val {
		...
		cluster.CompatibilityVersion = request.CompatibilityVersion
		cluster.ClusterType = request.ClusterType
		cluster.WorkLoad = request.WorkLoad
		cluster.ClusterStatus = models.CLUSTER_STATUS_ACTIVE_AND_AVAILABLE
...
func startAndPersistMons(clusterId uuid.UUID, mons []backend.Mon) (bool, error) {
	...
	for _, mon := range mons {
		...
		slu.SluId = *id
		slu.SluName = fmt.Sprintf("mon-%v", *id)
...
func addOSDs(clusterId uuid.UUID, clusterName string, nodes map[uuid.UUID]models.Node, requestNodes []models.ClusterNode) (bool, error) {
	...
	for _, requestNode := range requestNodes {
		...
		for _, storageDisk := range storageNode.StorageDisks {
			for _, devname := range requestNode.Disks {
				if storageDisk.Name == devname && !storageDisk.Used {
...
func persistOSD(clusterId uuid.UUID, nodeId uuid.UUID, diskId uuid.UUID, osd backend.OSD) (bool, error) {
	...
	var slu models.StorageLogicalUnit
	slu.SluId = *id
	slu.SluName = fmt.Sprintf("osd-%v", *id)
...
func (s *CephProvider) ExpandCluster(req models.RpcRequest, resp *models.RpcResponse) error {
	...
	for _, new_node := range new_nodes {
		if new_node.Options["mon"] == "Y" || new_node.Options["mon"] == "y" {
			var mon backend.Mon
	...
	ret_val, err := addOSDs(*cluster_id, cluster.ClusterName, updated_nodes, new_nodes)
	if !ret_val {
