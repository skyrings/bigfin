...
func UpdateCrushNodeItems(ctxt string, clusterId uuid.UUID, sluList1 map[string]models.StorageLogicalUnit) error {

	var sp1 models.StorageLogicalUnit
	sp1.Name = "osd.3"
	option1 := make(map[string]interface{})
	option1["device"] = "/dev/vdb"
	option1["node"] = "dhcp47-111.lab.eng.blr.redhat.com"
	sp1.Options = option1
	sp1.StorageProfile = "general"

	var sp2 models.StorageLogicalUnit
	sp2.Name = "osd.4"
	option2 := make(map[string]interface{})
	option2["device"] = "/dev/vdc"
	option2["node"] = "dhcp47-111.lab.eng.blr.redhat.com"
	sp2.Options = option2
	sp2.StorageProfile = "general"

	var sp3 models.StorageLogicalUnit
	sp3.Name = "osd.4"
	option3 := make(map[string]interface{})
	option3["device"] = "/dev/vde"
	option3["node"] = "dhcp47-111.lab.eng.blr.redhat.com"
	sp3.Options = option3
	sp3.StorageProfile = "general"

	var sluList map[string]models.StorageLogicalUnit
	sluList["sp1"] = sp1
	sluList["sp2"] = sp2
	sluList["sp3"] = sp3

	logger.Get().Debug("$$$sluList: %v", sluList)
	if len(sluList) == 0 {
		return nil
	}
	...
	logger.Get().Debug("$$$cNodes: %v", cNodes)
	...
	logger.Get().Debug("$$$sluDbs: %v", sluDbs)
	...
	for _, cNode := range cNodes {
		...
		if found {
			logger.Get().Debug("$$$Update Existing crush node")
	...
	logger.Get().Debug("$$$slus 2nd stage: %v", slus)
	...
	logger.Get().Debug("$$$rulesetmap: %v", rulesetmap)
	...
	for _, sprof := range sProfiles {
		...
		for _, node := range nodes {
			...
			if len(cNode.Items) > 0 {
				logger.Get().Debug("$$$cNode to creste: %v", cNode)
				...
				if !rootNodeFound {
					logger.Get().Debug("$$$root Node not found")
					...
					logger.Get().Debug("$$$Root Node to creste: %v", nNode)
					...
					cRule := backend.CrushRuleRequest{Name: sprof.Name, RuleSet: len(cRules), Type: "replicated", MinSize: MINSIZE, MaxSize: MAXSIZE}
					step_take := make(map[string]interface{})
					step_take["item_name"] = nNode.Name
					step_take["item"] = nNodeId
					step_take["op"] = "take"
					cRule.Steps = append(cRule.Steps, step_take)
					leaf := make(map[string]interface{})
					leaf["num"] = 0
					...
					emit["op"] = "emit"
					cRule.Steps = append(cRule.Steps, emit)
					logger.Get().Debug("$$$Rule Node to creste: %v", cRule)
					cRuleId, err := cephapi_backend.CreateCrushRule(monnode.Hostname, clusterId, cRule, ctxt)
					if err != nil {
						logger.Get().Error("Failed to create Crush rule for cluster: %s. error: %v", clusterId, err)
						continue
					}
					...
					logger.Get().Debug("$$$c Nodes after create: %v", cNodes)
				}
			}
	...
	for _, crNode := range cNodes {
		...
		logger.Get().Debug("$$$c updating...: %v", params)
