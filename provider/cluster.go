...
func configureOSDs(clusterId uuid.UUID, request models.AddClusterRequest, clusterMons []backend.CephInstallerMonDetail, t *task.Task, ctxt string) ([]string, []string, error) {
	sessionCopy := db.GetDatastore().Copy()
	defer sessionCopy.Close()
	var (
		failedOSDs    []string
