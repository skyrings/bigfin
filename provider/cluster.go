...
func (s *CephProvider) ExpandCluster(req models.RpcRequest, resp *models.RpcResponse) error {
	...
	if len(mons) > 0 {
		...
		for _, mon := range mons {
			for _, clusterNode := range cluster.Nodes {
				if mon.Node == clusterNode.Hostname &&
				(clusterNode.Options["mon"] == "Y" || clusterNode.Options["mon"] == "y") {
	...
	// Update nodes list for cluster
	if err := updateClusterNodes(cluster, request.Nodes); err != nil {
		*resp = utils.WriteResponse(http.StatusInternalServerError, fmt.Sprintf("Error updating cluster node list %v", err))
		return nil
	}
...
func updateClusterNodes(cluster models.StorageCluster, nodes []models.ClusterNode) error {
	for _, node := range nodes {
		found := false
		for _, clusterNode := range cluster.Nodes {
			if clusterNode.Hostname == node.Hostname {
				found = true
				clusterNode.Disks = append(clusterNode.Disks, node.Disks...)
				break
			}
		}
		if !found {
	...
	defer sessionCopy.Close()
	coll := sessionCopy.DB(conf.SystemConfig.DBConfig.Database).C(models.COLL_NAME_STORAGE_CLUSTERS)
	if _, err := coll.Upsert(bson.M{"clusterid": cluster.ClusterId}, bson.M{"$set": cluster}); err != nil {
		return err
	}
	return nil
}
