	...
	asyncTask := func(t *task.Task) {
		...
		for {
			...
			default:
				...
				t.UpdateStatus("Successfully created the cluster")
				t.Done(models.TASK_STATUS_SUCCESS)
				return
...
func configureOSDs(clusterId uuid.UUID, request models.AddClusterRequest,
	...
	for _, requestNode := range request.Nodes {
		...
		if expand {
			logger.Get().Error("Getting existing journals")

			// Get the journal disks from the node
			existingJournals, err := getJournalDisks(*uuid)
			if err != nil {
				logger.Get().Error(
		...
		for _, storageDisk := range storageNode.StorageDisks {
			for idx := consumedCount; idx < len(requestNode.Devices); idx++ {
				if storageDisk.Name == requestNode.Devices[idx].Name {
					if storageDisk.Used {
						logger.Get().Warning(
	...
	asyncTask := func(t *task.Task) {
		...
		for {
			...
			default:
				...
				t.UpdateStatus("Successfully expanded the cluster")
				t.Done(models.TASK_STATUS_SUCCESS)
				return
			}
		}
