...
func (s *CephProvider) CreateCluster(args models.Args, resp *[]byte) error {
	// log.Println("Inside rpc method")
	// log.Println(args.Request)
	// log.Println(args.Vars)
	...
	for _, node := range request.Nodes {
		...
		// log.Println(node_ips[node.Hostname]["cluster"])
		// log.Println(node_ips[node.Hostname]["public"])
	...
	if len(mons) == 0 {
		*resp = utils.WriteResponse(http.StatusInternalServerError, "No mons mentioned in the node list")
		return errors.New("ceph+provider: No mons mentioned in the node list")
	}
	// log.Println(request.ClusterName)
	// log.Println(request.ClusterId)
	// log.Println(mons)
