...
func (s *CephProvider) CreateCluster(req models.RpcRequest, resp *models.RpcResponse) error {
	...
	asyncTask := func(t *task.Task) {
		...
		if ret_val {
			...
			ok, err := cluster_up(*cluster_uuid, request.Name)
			if err != nil || !ok {
...
func (s *CephProvider) GetClusterStatus(req models.RpcRequest, resp *models.RpcResponse) error {
	...
	ok, err := cluster_up(*cluster_id, cluster.Name)
	if ok {
...
func cluster_up(clusterId uuid.UUID, clusterName string) (status bool, err error) {
