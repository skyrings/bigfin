...
func syncOsds(clusterId uuid.UUID, ctxt string) error {
	...
	for _, fetchedSlu := range fetchedSlus {
		fetchedSlusMap[fmt.Sprintf("%s:%s", fetchedSlu.Name, fetchedSlu.Options["device"])] = fetchedSlu
	}
	...
	for _, osd := range osds {
		...
		if _, ok := fetchedSlusMap[fmt.Sprintf("osd.%d:%s", osd.Id, deviceDetails.DevName)]; ok {
