...
func FetchPGDetails() (map[string]map[string]string, error) {
	monitoringConfig := conf.SystemConfig.TimeSeriesDBConfig
	statistics, statsFetchErr := cephapi_backend.GetPGDetails(monName, cluster.ClusterId)
	if statsFetchErr != nil {
	...
	metrics := make(map[string]map[string]string)
	currentTimeStamp := time.Now().Unix()
	for k := range pgsummary.ByOSD {
		for k1, v1 := range pgsummary.ByOSD[k] {
			metric_name := fmt.Sprintf("%s.%s.%s.by_osd.%s.%s", monitoringConfig.CollectionName, cluster.Name, skyring_monitoring.PG_SUMMARY, k, k1)
			statMap := make(map[string]string)
	...
	for k := range pgsummary.ByPool {
		for k1, v1 := range pgsummary.ByPool[k] {
			metric_name := fmt.Sprintf("%s.%s.%s.by_pool.%s.%s", monitoringConfig.CollectionName, cluster.Name, skyring_monitoring.PG_SUMMARY, k, k1)
			statMap := make(map[string]string)
			statMap[strconv.FormatInt(currentTimeStamp, 10)] = strconv.FormatUint(v1, 10)
			metrics[metric_name] = statMap
		}
