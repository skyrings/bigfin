	...
	if isRaiseEvent {
		...
		if message == "" {
			message = fmt.Sprintf("%v of cluster %v with value %v breached %v threshold of value %v",
				plugin.Name, cluster.Name, resourceUtilization, strings.ToUpper(applicableConfig.Type), thresholdVal)
		}
...
func UpdateThresholdInfoToTable(tEvent models.ThresholdEvent) (err error, isRaiseEvent bool) {
	...
	if err != nil {
		...
		if tEvent.ThresholdSeverity == tEventInDb.ThresholdSeverity {
			return nil, false
		} else if tEvent.ThresholdSeverity == "" {
			if err = collection.Remove(selectCriteria); err != nil {
				return fmt.Errorf("Failed to update the db, that %v of %v is back to normal.Err %v", tEvent.UtilizationType, tEvent.EntityName, err), true
			}
			return nil, true
		} else {
