...
func FetchOSDStats() (map[string]map[string]string, error) {
	...
	for _, osd := range statistics {
		metric_name := monitoringConfig.CollectionName + "." + cluster.Name + "." + skyring_monitoring.SLU_UTILIZATION + "_" + osd.Name + "."
	...
	for _, slu := range slus {
		for _, osdDetail := range osdDetails {
			if osdDetail.Name == slu.Name {
				...
				if utilization, ok := statsToPush[metric_name+skyring_monitoring.TOTAL_SPACE]; ok {
					existingUtilization, _ := strconv.ParseUint(utilization[currentTimeStamp], 10, 64)
					statsToPush[metric_name+skyring_monitoring.TOTAL_SPACE] = map[string]string{currentTimeStamp: strconv.FormatUint((existingUtilization + slu.StorageDeviceSize), 10)}
				} else {
					statsToPush[metric_name+skyring_monitoring.TOTAL_SPACE] = map[string]string{currentTimeStamp: strconv.FormatUint(slu.StorageDeviceSize, 10)}
				}

				if utilization, ok := statsToPush[metric_name+skyring_monitoring.FREE_SPACE]; ok {
					existingUtilization, _ := strconv.ParseUint(utilization[currentTimeStamp], 10, 64)
					statsToPush[metric_name+skyring_monitoring.FREE_SPACE] = map[string]string{currentTimeStamp: strconv.FormatUint(osdDetail.Available+existingUtilization, 10)}
				} else {
					statsToPush[metric_name+skyring_monitoring.FREE_SPACE] = map[string]string{currentTimeStamp: strconv.FormatUint(osdDetail.Available, 10)}
				}

				if utilization, ok := statsToPush[metric_name+skyring_monitoring.USAGE_PERCENT]; ok {
					existingUtilization, _ := strconv.ParseUint(utilization[currentTimeStamp], 10, 64)
					statsToPush[metric_name+skyring_monitoring.USAGE_PERCENT] = map[string]string{currentTimeStamp: strconv.FormatUint(osdDetail.UsagePercent+existingUtilization, 10)}
				} else {
					statsToPush[metric_name+skyring_monitoring.USAGE_PERCENT] = map[string]string{currentTimeStamp: strconv.FormatUint(osdDetail.UsagePercent, 10)}
				}
			}
		}
	}
...
func FetchObjectCount() (map[string]map[string]string, error) {
	...
	if statsFetchErr != nil {
		return nil, fmt.Errorf("Unable to fetch PG statistics from mon %v of cluster %v.Error: %v", monName, cluster.Name, statsFetchErr)
	}
	metrics := make(map[string]map[string]string)
	currentTimeStamp := time.Now().Unix()
	timeStampStr := strconv.FormatInt(currentTimeStamp, 10)
