...
func UpdateObjectCountToSummaries(ctxt string, cluster models.Cluster) {
	...
	if err != nil {
		...
		UpdateDb(bson.M{"clusterid": cluster.ClusterId}, bson.M{"objectcount": objCnt}, models.COLL_NAME_CLUSTER_SUMMARY, ctxt)
		objCnt, err := ComputeObjectCount(nil)
		if err != nil {
			logger.Get().Error("%s - Failed to update object count for system summary. Error %v", ctxt)
		} else {
			UpdateDb(bson.M{"name": monitoring.SYSTEM}, bson.M{"objectcount": objCnt}, models.COLL_NAME_SKYRING_UTILIZATION, ctxt)
		}
	}
}
