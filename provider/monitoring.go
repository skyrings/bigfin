	...
	/*
		...
		Logic:
			...
			3. Loop over the storage profile threshold events and for each storage profile event:
				3.1. Get the list of osds with threshold cross event for the current storage profile.
				3.2. Add the list of osds obtained in 3.1(empty or non-empty) to the field ImpactingEntities of the event related to the storage profile
				3.3. Raise threshold crossing event
				3.4. Remove the entry corresponding to the just processed storage profile from the map in 2
			4. Iterate over the remaining entries in the map fetched from 2 and raise osd threshold crossing event.
			   These are osds with no thresold crossing of associated storage profile
	*/
	...
	for _, spEvent := range storageProfileEvents {
		...
		impactingEntities := make(map[string][]uuid.UUID)
		for _, osdEvent := range osdEvents {
			osdIdFromEvent, osdIdFromEventError := uuid.Parse(osdEvent.Tags[models.ENTITY_ID])
			if osdIdFromEventError != nil {
				logger.Get().Error("%s - Failed to parse osd id %v from cluster %v.Error %v", osdIdFromEvent, cluster.ClusterId, osdIdFromEventError)
			}
			impactingEntities[models.COLL_NAME_STORAGE_LOGICAL_UNITS] = append(impactingEntities[models.COLL_NAME_STORAGE_LOGICAL_UNITS], *osdIdFromEvent)
		}
		spEvent.ImpactingEntities = impactingEntities
