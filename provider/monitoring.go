...
func (s *CephProvider) MonitorCluster(req models.RpcRequest, resp *models.RpcResponse) error {
	...
	metrics, err := FetchClusterStats(*cluster_id)
	if err != nil {
		*resp = utils.WriteResponse(http.StatusBadRequest, err.Error())
	}
	result, marshalErr := json.Marshal(metrics)
	if marshalErr != nil {
...
func FetchClusterStats(cluster_id uuid.UUID) (map[string]map[string]string, error) {
	...
	if clusterFetchErr != nil {
		...
		return nil, fmt.Errorf("Unbale to parse the request %v", clusterFetchErr.Error())
	}
	...
	if monNodeFetchErr != nil {
		...
		return nil, fmt.Errorf("Unbale to pick a random mon from cluster %v.Error: %v", cluster.Name, monNodeFetchErr)
	}
	...
	if statsFetchErr != nil {
		...
		return nil, fmt.Errorf("Unable to fetch cluster stats from mon %v of cluster %v.Error: %v", monnode.Hostname, cluster.Name, statsFetchErr)
	}
