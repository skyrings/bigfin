		...
		for {
			...
			default:
				...
				if request.Type != models.STORAGE_TYPE_ERASURE_CODED && len(request.BlockDevices) > 0 {
					createECPool(ctxt, monnode.Hostname, cluster, *poolId, request, t)
				}
...
func createECPool(ctxt string, mon string, cluster models.Cluster, poolId uuid.UUID, request models.AddStorageRequest, t *task.Task) {
